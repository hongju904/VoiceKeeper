...
#### 1. 감정 분석
- `음성 기반` wav2vec2.0를 이용하여 통화 녹음된 파일을 긍정, 분노, 공포, 슬픔 4가지로 분류
- `텍스트 기반` BERT를 이용하여 음성에서 잡아낼 수 없는 감정을 대화 속 내용에서 추측
- 두 결과에 가중치를 부여하여 합산. 분노, 공포, 슬픔의 값이 임계값을 넘으면 통화 녹음 필요 상황으로 판단
#### 2. 대화 요약
- Azure STT를 사용하여 음성 파일의 텍스트 변환
- koBERT를 이용한 요약문 모델
#### 3. 서버 통신
- Azure 서버의 Funtion App으로 모델 자동 작동
- Android 앱 내의 OkHttp 통신으로 트리거 생성
#### 4. 어플리케이션 구현
- 녹음 완료시 기기 외부저장소에 파일 저장 및 서버에 전송
- 멀티스레딩 HTTP Pooling으로 ANR 방지
...
## 🚩 개발 프레임워크
![image](https://user-images.githubusercontent.com/74871378/224705814-2afe2f27-5c33-449d-89c9-3def46fde1ef.png)
